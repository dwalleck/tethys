#!/bin/bash
# Create GitHub milestones for Stratify

echo "Creating GitHub milestones..."
echo ""

# Create milestones with due dates
gh api repos/dwalleck/Stratify/milestones \
  --method POST \
...
gh api repos/dwalleck/Stratify/milestones --jq '.[] | "- \(.title) (due: \(.due_on // "no due date"))"'
