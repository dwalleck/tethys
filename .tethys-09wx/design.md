...
| C5 | Stale file → exit 2 + `indeterminate: stale:` (mtime-only bump; size-change-mtime-restored variant at facade level) | bin test: touch file post-index; unit test: rewrite + restore mtime | `find -newer` logic by construction | 15m | pending | `affected_tests_cli::stale_is_indeterminate`; classifier unit `size_divergence_with_matching_mtime_is_stale` |
...
| C14 | CLI layer contains no direct DB/fs-metadata access | grep `rusqlite\|fs::metadata\|sqlite` in `src/cli/affected_tests.rs` == 0 | grep (mechanical) | 5m | pending | `affected_tests_cli::cli_layer_never_touches_db_or_fs_metadata` (build deviation: lives beside the CLI fences — `tests/architecture.rs` tests the coupling analysis, not repo conventions) |
