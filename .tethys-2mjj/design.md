...
| C8 | **Headline**: cold Rust fixture, full pipeline with LSP binds ≥1 ref with `strategy=lsp` (today: 0) | Run the new pipeline test against UNFIXED code — must fail (red); against fixed code — must pass; wrong direction falsifies design or test | The index's `refs.strategy` column, written by the pipeline, read by the test — independent of the wait's own logging | 30m write, ~30s run | pending — **red-first during build** | the same test, nightly (tethys-xpc4, verified open 2026-07-15) |
