...
| Neutral discovery | S3 | 16000 | Cargo/MSBuild discovery interface returns complete metadata/failures/cache evidence under grants, independently exercised through its public seam |
...
Sum: 22,800 changed lines (implementation+tests+fixtures/docs/evidence). Churn margin: 25%=5,700, for managed host packaging and schema/fixture integration; total28,500 >4,000. Each increment has its own observable seam and no dependence on later increments. S3's staged F14–F16 checkpoint contains15,281 changed lines across132 files against reconciled S2, including retained native and raw mutation evidence; the projection is revised to16,000. This affected-only update preserves all other slice estimates and the existing PR partition. Candidate/host/restore/cache ownership, protected parents, approved behavior and risk are unchanged; C13 remains mandatory. No unused or placeholder modules are introduced to meet a line target. Ownership changes still return to design approval. This partition is not permission to close the issue before S6.
...
**Diff estimate:** 16000 lines, amended for S3a's observed native repairs and retained execution/mutation evidence; production owners and independent discovery seam are unchanged.
...
**Files:** Cargo.toml, Cargo.lock, src/discovery/msbuild/{candidates.rs,host.rs,restore.rs,cache.rs}, tests/discovery_candidates.rs, tests/discovery_failures.rs, .github/workflows/ci.yml, docs/msbuild-evaluation.md, changelog.d/tethys-82a6-discovery.added.md and oracles/discovery_mutations.py; this plan, review-decisions.md, cache-order, Windows-path, SDK-restore and mutation execution evidence. Temporary diagnostics/probes and extra diagnostic CI output/steps are removed after native observations; the full native roster remains.
**Estimate:** one evidence-routed platform repair checkpoint.
**Diff estimate:** 6500 lines including retained native/mutation proof and diagnostic churn, replacing the disproven500-line projection. Existing production owner ranges remain unchanged and pass C13; this is a plan-size amendment, not an architecture or scope change.
