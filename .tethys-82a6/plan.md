...
**Files:** Cargo.toml, Cargo.lock, src/discovery/msbuild/{candidates.rs,host.rs,restore.rs}, tests/discovery_candidates.rs, .github/workflows/ci.yml; this plan, review-decisions.md, cache-order execution evidence, and temporary oracles/windows_path_probe.py. Fingerprint timings are removed after the actual Windows observation; the path probe remains diagnostic only and must be removed after its native comparison.
...
**Diagnostic disposition:** actual Windows job101651540392 attributes185.665–190.339 seconds to the first installation fingerprint and1.675–2.138 seconds to subsequent fingerprints (artifact484). No supervisor timeout occurred. Tagged fingerprint diagnostics and successful-test output capture are now removed; the original debug gate remains mandatory. Windows standard tests pass, including missing/outside path and unconditional whitespace fences, but native discovery still fails: Confirmed units have no Compile sources. Main owns an actual Windows ordinary/verbatim differential against direct MSBuild and the worker, then removal of the temporary probe/CI step. It does not replace or weaken the existing failing membership fence.
