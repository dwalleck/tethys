...
Approved inputs: route.md, spec.md, design.md (architecture approval verbatim “yes, I approve”, 2026-09-06). Structural route; bounded review repairs retain the approved contract as recorded in review-decisions.md. C0 passed including its early-commit negative control; C1–C14 have named independent oracles, mutations and owning checkpoints. No risk waiver. All seven issue acceptance criteria remain mandatory.

Baseline: repository upstream discovered as refs/remotes/origin/main. PR increments use stacked branches; standalone discovery is qualified on feat/tethys-82a6-discovery, and S4 uses feat/tethys-82a6-publication. Writers use isolated workspaces and disjoint ownership; Main integrates and owns shared seam edits/validation. No worker runs formatters, builds, lint or tests concurrently. Recorded checkpoints below incorporate checkpointed-build's judgments rather than treating the initial plan as proof.
...
| Neutral discovery | S3 | 17000 | Cargo/MSBuild discovery interface returns complete metadata/failures/cache evidence under grants, independently exercised through its public seam |
| Persisted evaluated discovery | S4 | 13000 | Index/reindex store and publish contexts/projects/units/membership and return typed partial coverage; snapshots readable without coupling |
| Evidence-aware coupling | S5 | 9000 | CLI/library coupling reports units/declarations/unknown metrics from persisted snapshot; all queries remain evaluation-free |
| Corpus qualification | S6 | 6000 | Reproducible pinned qualification/measurement tooling and records, requiring all earlier behavior; no product placeholder |

Sum:48,000 changed lines (implementation+tests+fixtures/docs/evidence). Churn margin:25%=12,000; total60,000 >4,000. Each increment has its own observable seam and no dependence on later increments. The S5 row now incorporates its recorded proof-retention amendment. S6's initial800-line estimate omitted corpus acquisition/standing manifests, phase timing, actual multi-unit scale, and executable resource/calibration checks; the6000-line projection covers those nonproduction responsibilities and retained evidence. No production responsibility, protected-parent allowance, acceptance threshold or PR boundary changes.
...
| src/unused_imports.rs | 350 | 350–370 | existing context and source-selection callers follow the published snapshot/canonical source domain; current351 | Rust query algorithm retained; no evaluation |
...
| src/error.rs | 211 | 211–240 | serialize existing bounded source errors for publication | no standing policy |
...
| src/discovery/msbuild/mod.rs | 0 | 550–800 | trust/evaluation workflow plus final bounded-inventory validation; current772 | no SQL |
| src/discovery/msbuild/candidates.rs | 0 | 440–682 | non-executing candidate/path parsing, bounded containers and scoped filter membership plus typed bounded traversal failures; current682 | no execution |
...
**Diff estimate:** 17000 lines, amended for S3a's observed native repairs and retained execution/mutation evidence; production owners and independent discovery seam are unchanged.
...
**Expected behavior:** the discovery dependency graph passes security policy; missing inside/outside Windows paths retain correct identities/reasons; native path presentation preserves Compile globs and ordinary SDK restore; equivalent canonical project identities retain valid restore currentness without accepting other projects.
...
**Stress fixture:** bare drive/verbatim prefix during missing-path resolution, missing ProjectReference, inside/outside controls, normal worker exit versus silent worker timeout, native SDK DLL argument spelling, and valid same-project alias restore versus another same-named physical project.
**Regression fence:** existing cargo-deny CI; discovery_candidates missing/outside manifest and xml_attribute_whitespace_preserves_distinct_paths_and_character_references; discovery_cache native metadata/launch fences; discovery_failures timeout, grant, later-restore/glob, existing_restore_requires_same_physical_project and authorized_restore_preserves_aliased_artifact_identity fences.
**Named mutation:** downgrade quick-xml to0.38.4 → observed RUSTSEC-2026-0194/0195; canonicalize bare Windows prefix → observed Incorrect function/wrong reason; normalize literal attribute whitespace → exact project/container mismatch in artifact455. F14's actual native DLL differential isolates success/no-assets versus successful restore. C7-lexical-project-identity and C7-lexical-generated-identity independently restore F16's defects; C7-lexical-captured-restore removes F17's captured physical identity. Each alias fence must fail at its acceptance assertion, then pass with source explicitly restored. No supervisor mutation or fix is justified.
**Complexity/production scale:** path fallback O(components), security-fixed attribute checks O(attributes), native presentation borrowed/no-I/O, and restore import classification O(imports+4) canonicalizations with a fixed four-output identity vector. Final restore exemptions reuse captured stamps in O(R log I + R log R + D log R) ordered operations per changed scope, for R restore paths, I captured inputs and D inventory differences; path comparisons depend on path length. They allocate only borrowed-reference sets and perform no new filesystem reads or projection on unchanged inventory. Existing output/deadline/resource bounds remain; full480-project/802-unit qualification remains S6.
...
**Module shape:** existing MSBuild coordinator and candidate/host/restore/cache owners only; no new public seam, grant or process policy. Existing S3 production tripwires and C13 remain applicable.
**Files:** Cargo.toml, Cargo.lock, src/discovery/msbuild/{mod.rs,candidates.rs,host.rs,restore.rs,cache.rs}, tests/discovery_candidates.rs, tests/discovery_failures.rs, .github/workflows/ci.yml, docs/msbuild-evaluation.md, changelog.d/tethys-82a6-discovery.added.md and oracles/discovery_mutations.py; this plan, review-decisions.md, cache-order, Windows-path, SDK-restore, final-currentness and mutation execution evidence. Temporary diagnostics/probes and extra diagnostic CI output/steps are removed after native observations; the full native roster remains.
**Estimate:** one evidence-routed platform repair checkpoint.
**Diff estimate:** 7500 lines including retained native/mutation proof and diagnostic churn; the staged F18 checkpoint reaches7,439 lines across108 files since S3a began. Existing production owner ranges remain unchanged and pass C13; this is a plan-size amendment, not an architecture or scope change.
...
**Diagnostic disposition:** actual Windows job101651540392 attributes185.665–190.339 seconds to the first installation fingerprint and1.675–2.138 seconds to subsequent fingerprints (artifact484). No supervisor timeout occurred. Those tagged timings and successful-test output capture are removed; the original debug gate remains mandatory. Windows standard tests pass, including missing/outside path and unconditional whitespace fences. Native job101659036681 then proves the missing-source mechanism on both SDK8 and VS17.14: ordinary paths preserve Compile globs, while verbatim paths silently produce empty globs; literal-item controls pass for both. All16 direct/worker observations are retained in evidence/windows-path-4b037e1.json. The temporary Python path probe and its CI step are now removed.
...
**Cache-order repair evidence:** run34093399905 passes Cargo Deny and all ordinary platform tests, but Ubuntu/macOS native discovery fails before fingerprinting with ToolchainUnavailable/ENOENT. Both lanes restored a nonmatching Rust cache after worker preparation. Executing the unchanged pinned action6323deb's cleanTargetDir on a copied real distribution deletes its DLL while preserving directories; the existing public-seam cache-input fence goes0→100 with the exact ENOENT, then0 when publication follows cleanup. Raw logs and action/source provenance are in evidence/cache-order-3d69823. Move existing Rust setup/cache/nextest before preparation; no product change, weaker assertion, new cache key or packaging-path convention. The isolated probe/dependencies were removed after recording proof. Actual CI remains required.

**Native path repair contract:** use dunce1.0.5's borrowed, no-I/O filesystem-equivalent presentation only at managed evaluation/restore boundaries. Its Apache-2.0 alternative is already allowed; no license-policy change. Keep canonical internal project/workspace/host identity, strict presented-path wire echo and existing selected-host validation; restore verified canonical project/host identity in accepted evaluation records before cache/domain use. Preserve native metadata strings and authored global values. Increment the existing opaque cache recipe to invalidate pre-fix successful-but-empty results. Existing native membership, zero-evaluator cache and restore fences remain unchanged and must pass on actual Windows; no empty-source special case or relaxed assertion.
...
**Restore boundary repair — F14/F15/F16:** actual Windows run34105613204 demonstrates the SDK DLL argument cause independently of tethys: verbatim exits0 with no assets; ordinary exits0 and writes them. The production command now presents that DLL through dunce. Restore validation compares canonical project identity and uses the native dgspec key; canonical generated/import comparison excludes the same physical outputs without losing native watch paths or unrelated freshness checks. The real alias fence exposed both lexical boundaries before repair. Artifact570 passes fmt/clippy/deny,1156 ordinary tests,678 native-enabled checks,18 doctests, public discovery/Cargo smoke and C13. Final source mutation replay in evidence/discovery-mutations-e8_7dw4z passes9 baseline fences, catches10 behavioral failures from9 named mutations, and explicitly restores/passes all10 affected cases. Actual post-fix Windows is still Main's required next checkpoint; no S4 advance is authorized by local proof.

**Captured inventory repair — F17:** Windows050fbf9 reaches final currentness but still rejects three SDK restores; evidence/windows-currentness-050fbf9.json preserves the observed boundary and passing legacy controls. The real Unix aliased-artifact fixture reproduces it. Final validation now derives native/canonical names from this scope's captured restore stamps, preserving original watch paths and refusing to canonicalize arbitrary new inventory aliases. Artifact597 passes fmt/clippy,1156 ordinary tests,679 native-enabled checks,18 doctests, public discovery/Cargo smoke and C13. The scoped mutation runner records selected names explicitly; discovery-mutations-uyjzn7zr catches restore-grant bypass and lexical-only captured identity, with both baseline and explicit restoration controls passing. Prior unaffected mutation proof is retained under the applicability record in review-decisions.md. Actual Windows post-fix acceptance remains mandatory before S4.

**Timestamp fixture repair — F18:** native6ca1b92 passes644/645 tests; only the no-op restore fixture's read-only File::open handle fails set_modified with PermissionDenied. Production restore/currentness logic is unchanged. The fixture now opens the existing project with nontruncating write access, retaining its future timestamp and every assertion. Artifact609 passes fmt/clippy and all16 native failure tests; discovery-mutations-vw3sdhpm supplies the changed fixture's fresh baseline, restore-grant-bypass red and explicit restored green. Actual Windows must continue through the no-op/cached/forced sequence before S3a can pass.

**Accepted S3/S3a platform checkpoint:** d9119ffdf5f29b81bf56aeaa94ba575f29fe96e4 passes all17 jobs in run34116637039. Native Windows executes645 tests; native Ubuntu and macOS each execute655, all passing with zero skipped. The full no-op/cached/forced restore sequence and legacy/default/filter controls complete. evidence/platform-d9119ff.json retains native summaries/control lines, URLs and raw-log hashes. The nine S3/S3a gate states are PASS with fresh or valid retained evidence in review-decisions.md; corpus/final assembled obligations remain S6. Upstream fetch reveals no unseen commits. This evidence-only checkpoint does not change runtime assumptions; S4 may proceed on its independently mergeable stacked increment.

## S4: Persist and publish evaluated metadata during index and reindex

**Claim IDs:** C8; C3/C7/C13 as extended by the approved F23 amendment; C6 for the evidence-routed native lifecycle repair.
**Expected behavior:** one physical C# syntax set participates in multiple units while Rust retains logical source identities; contexts/project moves/failures replace old metadata coherently; index/reindex reevaluates or validates on every invocation; options/grants reach adapter; all bounded evaluation failures give nonzero status without suppressing source-only data.
...
**Complexity/production scale:** O(files+units+memberships+refs+inputs) bulk snapshot insert/replace, including one physical-file identity map; 802 units,15,124 physical files; memberships can multiply across units but syntax isn't copied. Maximum6GiB RSS/10GiB database+sidecars; no row-by-row repeated full query. Serialization bounded by snapshot input records.
**Wall budget/phase:** every index/reindex invocation; corpus total30min qualification, perunit60s. No query-triggered work.
**Module shape:** db/discovery owns SQL, discovery owns execution; IndexOptions becomes owned Clone and all callers migrate atomically; ModuleContext carries immutable context. Approved F23 deepens the existing private ModuleResolver with SourcePathIdentity policy; neutral selection/query callers consume it. S4 projections indexing≤1440, lib≤1220, resolve≤1370, reindex≤400; baseline growth caps do not increase and S5 still extracts architecture. `python3 .tethys-82a6/oracles/module_shape.py --stage S4` → C13 PASS.
**Files:** src/db/discovery.rs, db/schema.rs, db/mod.rs; types.rs, error.rs, lib.rs, indexing.rs, reindex.rs, resolve.rs, unused_imports.rs, languages/module_resolver.rs; discovery/types.rs, discovery/mod.rs and msbuild/{mod.rs,cache.rs} for canonical grants/captured-input projection; main.rs, cli/index.rs; every IndexOptions/ModuleContext consumer found by LSP; tests/msbuild_discovery.rs, discovery_cli.rs, revision_publication.rs, existing discovery fences and broken contract callers; existing smoke/mutation/shape oracles, CI native gate and atomic docs/fragment.
**Estimate:** one persistence/integration increment.
**Diff estimate:**13000 lines including retained raw native proof; final census and rationale are in the review-size partition.
...
**S4 implementation decisions:** C8's independent membership uses PK(unit_key,path) with nullable files FK/ON DELETE SET NULL: source parse failure still removes stale syntax without erasing evaluated Compile evidence. The cache owner projects its already captured stamps into separately ordered input scopes, including invalidated observations; database code never decodes private receipts. Requested grants are explicit non-secret booleans. One Arc snapshot owns Cargo and evaluated context; a run-local Drop guard restores the prior context on error/unwind while the existing Revision restores SQL. Query construction loads persisted metadata only; fresh/rebuild construction retains Cargo fallback. Source selection unions evaluated entries with the source walk and retained records, applying the approved language-path identity policy: physical C# deduplication, logical Rust compatibility. Schema2 rejects prior layouts before mutation.

**S4 impact and technical placement correction:** initial fresh index found163 files/3,544 symbols/30,799 references. Trusted callers identify Tethys::index; broad callers identify19 entry/test consumers, and LSP identifies53 IndexOptions references plus27 ModuleContext references. LSP/grep found unused_imports.rs's existing query-side constructor, omitted from the initial ledger. Its initial context argument migration and subsequent F20 source-selector migration retain the Rust query algorithm and introduce no evaluator or new language policy. Root CrateInfo and IndexError receive serde for approved persisted records, not a second model. F20/F21's fresh index found164 files/3,623 symbols/32,000 references; LSP and both tethys tiers identify all4 source-selector callers. Source bindings remain syntax-only.

**S4 initial proof:** artifact626 observes the old CLI incorrectly returning0 for denied-trust discovery; the same fence passes in artifact650 after source-only publication and exit1. Artifact650 also executes both native C8 transitions and batch/stream rollback/Rust golden controls:15 passed, excluding only the separately qualified worker-package oracles. Broader quality, full native input-fingerprint fences, C8 mutants, source census and802-unit/15,124-file measurement remain required.

**S4 bounded repairs/final census:** F19–F23 and applicable checks are owned by review-decisions.md. Shared selection reconsiders existing indexed source rows independently of current metadata, so withdrawal cannot strand stale syntax behind walk exclusions. `evidence/s4-source-census.json` uses C13's exact cfg(test)-excluded physical-line calculation: indexing1405, lib1201, module_resolver436, resolve1343, reindex379, unused_imports351, db/discovery345. Existing architecture extraction in S5 still lowers indexing. The observed bounded-I/O repair crosses candidate570/coordinator750 review tripwires; source review confirms their existing non-executing traversal/workflow owners remain cohesive. Projections become640/800, respectively, without a new owner, dependency, language leak or waiver. C13, independent conformance and S4 measured resource bounds pass; committed native platform acceptance remains mandatory.

**F23 approved repair plan (2026-09-07):** design.md records verbatim approval, "Approve existing seam". Inherit the full S4 contract and nine gates. Source writer owns only lib.rs, indexing.rs and languages/module_resolver.rs; independent fence writer owns concurrency_and_filesystem.rs and symlink_boundary.rs. Main owns integration, oracles, documentation and verification. Add private SourcePathIdentity::{Logical,Physical} and ModuleResolver::source_path_identity; Rust/CSharp implementations select their policy, consumers remain language-neutral. Reuse lexically_normalize, existing canonicalization/diagnostics, HashSet selection and the static resolver factory; no dependency or public API addition. Expected module_resolver430–470 production lines, indexing1405–1440, lib1190–1220, no raised growth caps. Additional bounded repair allowance250 changed lines; actual increment/cumulative size must still be checked against the existing independently-green PR partition before commit.

**F23 impact/proof:** fresh LSP finds5 discover_files locations (declaration plus indexing, two reindex callers and unused_imports),21 relative_path locations across lib/indexing/resolve/reindex/unused_imports, and6 ModuleResolver locations (declaration, registry return, two implementations, resolve import and stored trait reference). Grep supplies the resolution-slice recall net; new policy has no pre-existing callers. Existing Rust symlink fences are RED on S4 (artifact714), GREEN on untouched2c002f5 (7 tests, artifact722). Keep those expectations; turn the introduced Rust-canonical fence into the intended C# alias/containment/dependency fence. New inverse Rust/CSharp policy mutations and localized C13 owner mutation are specified in the approved design. Run full ordinary/native suites, actual CLI/SQL, applicable scale, renewed isolated conformance, then reconcile all nine gates. No S5 advancement on a failed gate.

**S4 accepted local/native result:** all nine gates pass as recorded in review-decisions.md. F23's assembled suite, packaged CLI/SQL oracle, frozen Cargo/public discovery smoke, reciprocal policy/effective-owner mutants and802-unit/15124-file scale measurement are complete. Native F24–F29 repairs are verified; Darwin zombie-group mutation is red/restored-green, overflow is100/100 green, and original Windows cleanup succeeds. After removing diagnostics, artifact838 passes1167 ordinary tests,18 doctests, fmt/clippy and C13; committed4c8b397dcbbf1c5ddbbbca8040a39b1186dd44b4 passes all17 jobs in run34158268592. PR47 publishes the independently qualified increment. Fresh upstream fetch shows no unseen commits. S5 may now proceed; S6 corpus/performance and final assembled requirements remain mandatory.
...
**Regression fence:** issue-local oracles/qualification.py, deterministic threshold-boundary fixture, oracles/qualification_cadence_fence.py and oracles/qualification_deadline_fence.py; timeout fence retained in tests/discovery_failures.rs.
**Named mutation:** replace qualification cap rejection with unconditional acceptance → C12 above-cap control accepted; make the roster checks in qualification.py's arguments() unconditional acceptance → the cadence fence reports the seven misassigned/partial selections as accepted; replace Runtime's absolute deadline with an effectively infinite one → the deadline fence reports a non-shrinking budget, an expired deadline that still returns, and a second subprocess that receives a fresh budget instead of the remainder; remove worker deadline → controlled silent-worker timeout fails.
...
**Files:** .tethys-82a6/oracles/{qualification.py,qualification_cadence_fence.py,qualification_deadline_fence.py,qualification_limits.py,qualification_runtime.py,qualification_driver.rs,qualification_fixtures.py,qualification_corpus.py,worker_qualification.py,discovery_smoke.py,module_shape.py,ledger.json}; tests/fixtures/msbuild/qualification corpus/expectation manifests; existing workflow qualification wiring; measurement records; approved atomic docs/fragment as relevant.
**Estimate:** full host/corpus qualification increment.
**Diff estimate:** 6000 lines including nonproduction runner/helpers, manifests and retained execution proof; existing S6 PR boundary retained.
...
- `python3 .tethys-82a6/oracles/qualification.py --corpus --cadence pr` and `--check-boundaries --cadence pr` → fail closed: a lane the cadence does not own, and a partial selection that is not PR acceptance.
...
**Cadence assignment and accounting:** cadence selects work; the runner enforces the roster instead of trusting the label. `pr` owns committed fixtures with an explicit authored `--shapes` subset plus boundary arithmetic; `nightly`/`release` own the pinned corpus and platform-required legacy and require exactly their calibrated repetition count; formula shapes, density capture and instrumented/control proof remain `qualification`. A selection missing a cadence's required lane, or claiming a lane that cadence does not own, fails before any runtime work. The cadence cap binds the aggregate invocation from argument validation through the final lane — preparation, measured product invocations, same-root transparency controls and canonical projection — reported as separate preparation/lane/product/control/overhead seconds; the cap is neither reset per lane nor narrowed to product observations. The eligible-cache transition sequence is one authored sequence: `--repeat` does not apply, its records carry no calibrated workload identity, and it is declared excluded from ratio grouping rather than silently omitted.

**Implementation critique and ownership:** qualification.py does not yet exist; recorded S4 publication stress evaluates802 single-unit projects, not S6's480-project/802-unit shape, and S5's SQL-authored scale is query-only. Main owns orchestration, arithmetic limits, workflow and assembled proof. A concrete nonproduction runtime owns process measurement, phase timing, public index/update mode selection and canonical snapshot projection; fixture and corpus modules consume that same runtime. Existing measured_cli supervision is reused rather than duplicated. Formula fixtures must exercise actual evaluation at18 TFMs,480/802 and208/356 shapes with the recorded source counts.

**Phase-timing mechanism:** the CLI has no forced batch/stream selector or complete discovery/reindex timing surface. A qualification-only Rust caller exercises the existing public library methods/options. Any necessary timing instrumentation is applied solely to a disposable source copy, never the shipping tree; retain original/patch/binary hashes and require canonical agreement with an uninstrumented build on the same inputs. Report discovery, evaluation calls, downstream indexing and full invocation time separately; do not label subtraction of unrelated invocations as phase timing.

**Missing-input discipline:** immutable corpus pins come from approved tethys-umjq; SDK/context and expected-standing manifests must be established through direct-host/filesystem authority before qualification, not inferred from tethys output. No reviewed14-observation calibration manifest has been located in this checkout. Recover existing provenance first; never invent baseline values or claim calibrated-ratio acceptance from an unreviewed or differently scoped capture.

**Corpus authority (2026-09-08):** the requester approved committing the four pinned captures (~59MB) and signed off on the derived expected standing. `tests/fixtures/msbuild/qualification/` now holds `<id>.linux.sdk.capture.json.gz` plus `<id>.linux.sdk.expected.json` with `review: reviewed` (reviewer Daryl Walleck) for Avalonia 11.3.7, xUnit v3-3.2.2, Newtonsoft.Json 13.0.4 and MSBuild v18.8.2. `corpus_review.py` derives the projection from the committed native captures: enumeration failures map MSB4236/MSB4242/NETSDK1045 to sdk-unresolved, NETSDK1147/NETSDK1080 to toolchain-unavailable and other native codes to evaluation-failed; an authorized Restore with non-zero exit maps the project and its units to restore-failed; confirmed projects require every enumerated framework slot with exit 0 and either Restore exit 0 with unchanged final inputs or no Restore requirement. Observed standing: avalonia 104 projects/178 units (159 confirmed), msbuild 85/86 (80), xunit 47/85 (83), newtonsoft-json 4/21 (21); expected index exit 0 for every corpus (the invoked qualification driver exits nonzero only on fatal discovery failure, not on typed partial coverage). The corpus lane remains the falsifier: a typed-reason disagreement fails qualification rather than adjusting the expectation. The reviewed density baseline and the reviewed fourteen-observation calibration authority remain absent and are not waived.
