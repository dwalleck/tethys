...
| Persisted evaluated discovery | S4 | 7000 | Index/reindex store and publish contexts/projects/units/membership and return typed partial coverage; snapshots readable without coupling |
...
Sum:29,200 changed lines (implementation+tests+fixtures/docs/evidence). Churn margin:25%=7,300; total36,500 >4,000. Each increment has its own observable seam and no dependence on later increments. S3's qualified F18 checkpoint contains16,934 changed lines across148 files against reconciled S2; its projection remains17,000. S4's final projection is7,000: the precommit census is6,874 changed lines across75 files, comprising1,659 production,1,193 tests,401 oracles,484 docs/config and3,137 retained evidence lines, before this footprint record. The earlier3,200 projection understated retained mutation/native/scale evidence and bounded repair fences. This is a size correction, not a new responsibility/interface or risk waiver; normalized metadata publication remains one independently qualified increment. Existing owners, protected-parent limits and PR partition remain unchanged. C13 remains mandatory; no unused modules or placeholders satisfy a size target. Ownership changes return to design approval. This partition does not permit closing the issue before S6.
...
**Claim IDs:** C8; C3/C7/C13 as extended by the approved F23 amendment.
...
**Diff estimate:**7000 lines including retained raw proof; final census and rationale are in the review-size partition.
...
**S4 final local result:** all nine local gates pass as recorded in review-decisions.md. F23's full assembled suite, actual packaged CLI/SQL oracle, renewed frozen Cargo/public discovery smoke, reciprocal policy mutants, effective-owner C13 mutant and802-unit/15124-file scale measurement are complete. Post-cleanup43 focused checks and C13 pass; docs and changelog move atomically with behavior. Next: commit the independently qualified publication increment, open its stacked PR and obtain native CI acceptance before S5. S6 corpus/performance requirements remain unchanged.
