...
**Files:** Cargo.toml, Cargo.lock, src/discovery/msbuild/{candidates.rs,host.rs,restore.rs,cache.rs}, tests/discovery_candidates.rs, .github/workflows/ci.yml, docs/msbuild-evaluation.md and oracles/discovery_mutations.py; this plan, review-decisions.md, cache-order and Windows-path execution evidence. Both temporary diagnostics/probes and their extra CI output/step are removed after native observations.
**Estimate:** one evidence-routed platform repair checkpoint.
**Diff estimate:** 500 lines including retained native proof and diagnostic churn; production owner ranges remain unchanged.
