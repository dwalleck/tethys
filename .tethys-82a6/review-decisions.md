...
**Current status:** S4 remains accepted at diagnostic-free4c8b397. S5's assembled local qualification passes after F30–F32; committed native acceptance remains required before advancing. S6 and final assembled qualification remain required before issue closure.
...
**Scope of acceptance:** local S5 proof only. The workflow now runs the same actual coupling oracle on native platforms, adding the selected VS classic host on Windows. Committed native results are still required; no Windows/classic or macOS pass is inferred from Linux. S6 and final issue acceptance are not waived.
...
- **Local nine-gate reconciliation:** affected tests PASS, stress PASS, module shape PASS, budget PASS, regression fences PASS, behavioral/structural mutations PASS and restored fences PASS retain the source-pinned S5 evidence: no production, permanent test, query input or measured Linux path changed. Falsifier and implementation/oracle agreement PASS locally use the fresh exact framework-property differential for the corrected fixture input. Native classic coupling acceptance remains outstanding and blocks S5/S6 advancement.
- **Cleanup/commit:** no diagnostic product code or permanent test is added; the throwaway differential runner is removed. This bounded proof-input correction and its evidence travel atomically. Re-run committed native qualification without weakening any assertion or timeout.
