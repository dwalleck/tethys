...
| F26 | Native macOS overflow fence returns a non-Overflow outcome but its assertion hides the actual variant. | macOS CI / Main / MacOverflowDiagnosis | Verified | Job101819543036 executes1166 checks:1165 pass, only overflowing_stderr_terminates_process_group fails at host.rs:1222 in0.147s. Pinned fixture timeout is5s; F25 passes. | Modify | Capture supervision, cleanup and returned outcome in test-only diagnostics while preserving fixture, deadlines and strict Overflow assertion; choose a repair only after the native cause is observed. | Cleanup Io masking, earlier pipe/wait Io and short producer output remain hypotheses, not accepted diagnoses. S4 native acceptance remains blocked. |
...
- **Shape:** evidence/s4-shape-mutation.json records current baseline PASS, omission of unused_imports.rs ownership red at that exact path, byte-exact restoration and restored PASS, with source hashes. F23's proposed ownership/interface change still needs approval and renewed conformance.
- **Evidence disposition:** S4 remains FAIL. Main owns implementation, new/invalidated language-identity proof, full assembled/platform gates and atomic documentation after approval. Existing C8 membership mutants remain meaningful for their unchanged fixtures; changed fences or mutation applicability require fresh red/restored-green proof.
...
- **Disposition:** macOS acceptance remains failed until diagnosed and repaired. Other production, mutation, conformance and scale conclusions remain applicable to unchanged bytes. Temporary diagnostics must be removed after the discriminator; no fresh production-scale run is claimed by this probe.
