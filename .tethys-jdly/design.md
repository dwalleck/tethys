...
| C8 | On zbus 4.4.0: every Definite row is rustc-confirmed (precision 100%); Path B adds 0 rows. *(Re-baselined during build — original "exactly 5" prediction was made without running the tier SQL on zbus: a non-deprecated `struct_field` named `auth_mechanisms` conservatively demotes 3 true sites to Maybe; see `audit.md`.)* | Run CLI on zbus index; compare to rustc warning list | rustc `--force-warn deprecated` (captured in findings.md) | 30m | **passed** (audit.md: Definite 2/2 rustc-true, Path B 0, clean 2/2 rustc-true) | `deprecated_callers::same_file_phantoms_never_definite` (embeds the zbus phantom shape + 53iv TRIPWIRE); zbus run = one-shot audit, `manual` (user-approved 2026-07-03) |
