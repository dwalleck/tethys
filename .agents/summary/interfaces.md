...
| `impact <target>` | `-s/--symbol`, `-d/--depth <N>` (default 50), `--lsp` | Impact of changing a file (or symbol with `--symbol`), partitioned by minimum depth. Symbol impact is Index-backed and rejects `--lsp`. |
...
classDiagram
    class Tethys {
        ...
        +get_impact(file, depth) Result~FileImpact~
        +get_symbol_impact(symbol, depth, call_edges) Result~SymbolImpact~
