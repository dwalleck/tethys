...
/// Acceptance criterion 2: indexing the same workspace twice produces an
/// identical `file_deps` state — same rows AND same `ref_count` per row.
///
...
#[test]
fn file_deps_stable_across_repeated_indexing() {
    ...
    // circuit on the second run. Without this the second `index()` would
    // skip the file entirely and `insert_file_dependency` would never be
    // invoked — masking the bug class.
    ...
    assert_eq!(
        ...
        "file_deps ref_count_sum drifted; first={} second={}. \
         Pre-rivets-lcb6 the UPSERT incremented ref_count on every re-index, \
         so this sum grew without source changes.",
        first.ref_count_sum, second.ref_count_sum
    );
}

/// Acceptance criterion 3: removing a `use` statement and re-indexing
/// removes the corresponding edge from `file_deps`. Pre-fix the stale edge
/// would persist because the table was never cleared.
#[test]
fn file_deps_removed_when_use_statement_deleted() {
    let dir = tempfile::tempdir().expect("create tempdir");
    build_two_crate_workspace(&dir);
    let mut tethys = Tethys::new(dir.path()).expect("Tethys::new");
    ...
    assert!(
        ...
        "expected file_deps edge count to drop after removing the cross-crate \
         `use`; got before={} after={}. Pre-rivets-lcb6 the stale edge would \
         persist because the table was never cleared.",
        before.row_count,
        after.row_count
    );
}
